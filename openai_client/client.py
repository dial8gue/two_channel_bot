        ...
        try:
            ...
            response = await self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "Ты - аналитик групповых чатов с чувством юмора. Ты СТРОГО следуешь указанному формату ответа. Используй Markdown для форматирования: *жирный текст* только для заголовков разделов."
                    },
                    {
                        "role": "user",
                        "content": prompt
...
ВАЖНО: Ты ОБЯЗАН строго следовать указанному формату. Используй Markdown: *жирный* только для заголовков разделов (1., 2., 3., 4.).
...
ВАЖНО ПРИ УПОМИНАНИИ ПОЛЬЗОВАТЕЛЕЙ: Всегда используй формат @username. Если в username есть подчеркивания, экранируй их обратным слешем (например: @user\_name вместо @user_name), чтобы Markdown корректно отображал имя.

НАЧНИ ОТВЕТ СРАЗУ С ПЕРВОГО ПУНКТА. НЕ ДОБАВЛЯЙ ВСТУПЛЕНИЙ ИЛИ ЗАКЛЮЧЕНИЙ."""
        return prompt
